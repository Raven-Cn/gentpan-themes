<?php get_header(); ?>
        <main>
          <div class="Qb Rb J Sb Fb Tb">
            <header class="Eb m Vb Wb Xb Yb Zb ac bc cc Fb F G H I Bb">
              <h1 class="Ub s t u v">
                <span class=r>分类：</span><?php the_category(', ') ?></h1>
              <div><?php $category = get_category(get_query_var('cat')); echo '共 '. $category->count.' 篇文章'; ?></div></header>
			...
			<?php if(have_posts()) : while (have_posts()) : the_post(); ?>
              <article class="m Vb Wb Xb Yb Zb ac bc cc vf">
                ...
                <picture>
                  <source srcset="<?php echo get_the_post_thumbnail_url( null, array(240, 240) ); ?>" alt="<?php the_title(); ?> media="(max-width: 480px)">
                    <img class="Ic if wf xf Bf Cf Df Ef sc Hb Ff Gf Hf If Jf" src="<?php echo get_the_post_thumbnail_url( null, array(240, 240) ); ?>" alt="<?php the_title(); ?> alt="<?php the_title(); ?>" loading=lazy></picture>
                </a>
                <div class="m F G H I kb Bb">
                  <h3 class="s t Fb v c">
                    <a class="Lb Mc" href=<?php the_permalink() ?>><?php the_title(); ?></a></h3>
                  <div class="Fb Jc Cb">2023 <?php echo mb_strimwidth(strip_tags(apply_filters('the_content', $post->post_content)), 0, 240,"..."); ?></div>
                  <div class=Kb>
